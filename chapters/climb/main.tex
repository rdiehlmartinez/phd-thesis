...
\paragraph{Why might curricula fail in this setting?} Several factors may explain these negative results. First, the 10-million-word training set may be too small to benefit from curriculum structure --- the model may need exposure to all available data rather than a curated subset at any given time. Second, the difficulty metrics I explored (frequency, perplexity, source) may not capture the relevant notion of complexity for language learning. Third, modern language models may be sufficiently robust that they can extract useful signal from randomly ordered data, making careful sequencing unnecessary. Finally, the masked language modelling objective itself may be well-suited to learning from diverse, unstructured input, reducing the potential gains from curriculum approaches.

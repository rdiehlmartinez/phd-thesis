...
In this chapter, I build on the analytical frameworks introduced in Chapter 5 and provide a detailed investigation into the convergence behaviour of language models across scales. Specifically, I focus on how internal representations, especially the outputs of attention and feed-forward layers, evolve during training. I use tools like CKA and a new metric I term Proportional Effective Rank (PER) to examine how quickly and smoothly different layers in a model converge to their final state, and how this varies with model size.
...
Each layer in a transformer updates the model's residual stream by composing two subcomponents: multi-head self-attention ($\attention$) and a feed-forward network ($\mlp$). These updates can be expressed as:
\begin{align}
...
Intermediate checkpoints are available every \q{1}{\thousand} steps and at log-spaced intervals early in training. I consider models up to \q{2.8}{\billion} parameters (i.e., \sevenmil, \sixmil, \fourmil, \onebil, and \twobil) evaluated at the following steps: \integer{0}, all log-spaced steps $\{1, 2, 4, \smalldots, 512\}$, \q{1}{\thousand}, \q{3}{\thousand}, and then every \q{10}{\thousand} steps up to \q{143}{\thousand}.
I evaluate each checkpoint on the last batch of the training set and collect its activations. Note that the Pythia model suite was trained on the Pile \citep{gao2020pile}. Both the preprocessed training data and intermediate checkpoints are publicly available.%\footnote{\href{https://github.com/EleutherAI/pythia}{\myemph{github.com/EleutherAI/pythia}} (Apache License 2.0).} 
...
Noticeably, the packing process implies that the second half-epoch of deduplicated data contains the same documents but not necessarily the same sequences. By design, each sequence can pack multiple documents and tokens can attend across document boundaries.
...
\begin{table}[!t]
    ...
    \caption{Details on the architecture and training hyper-parameters for models in the Pythia suite used in this paper. $\numlayers$ is the number of layers, $\residualdim$ is the dimension of the residual stream. The number of hidden dimensions per head is simply the number of heads divided by the number of dimensions in the residual stream.}
...
To ascertain the degree of variability in $\cka$ and $\per$, I also conduct two analyses on the last $\mlp$ layer of each model. For $\cka$, I subsample 50\% of the activation data 10 times at four checkpoints (1000, 43000, 93000, and 143000), computing the mean and standard deviation across subsamples. For gradient PER, I compute the metric using 10 consecutive gradient snapshots centred around each checkpoint step and report the corresponding statistics.\footnote{For the final checkpoint (143000), I use only the five preceding gradient steps.} A similar analysis of weight $\per$ variability is not possible, as I stored only a single weight snapshot per checkpoint; however, I expect its variability to be comparable to that of the gradient $\per$, given that weights are updated directly from the gradients.

\begin{figure*}[h!]
    \centering
    \includegraphics[width=0.85\linewidth]{chapters/tending-towards-stability/figures/last_layer_mlp_styled.pdf}
    \vspace{-10pt}
    \caption{Variability analysis for the last $\mlp$ layer across Pythia model scales. \textbf{Left:} $\cka$ similarity (current vs.\ final checkpoint) with error bars showing standard deviation across 10 subsamples of 20\% of the activation data. \textbf{Right:} Gradient $\per$ with error bars showing standard deviation across 10 consecutive gradient snapshots around each checkpoint.} %The consistently tight confidence intervals across model scales and training stages indicate that both metrics are robust to sampling noise.}
    \label{fig:cka-per-variability}
\end{figure*}

As shown in \cref{fig:cka-per-variability}, the resulting standard deviations are consistently small across all model scales and training stages. The tight error bars, particularly for gradient $\per$, where standard deviations rarely exceed 0.005, suggest that these learning dynamics metrics are robust to sampling noise and that such noise is unlikely to significantly affect the analysis.

However, it is also important to caveat that the Pythia models use different learning rates across scales (see \cref{tab:model_hparams}), with smaller models trained at higher learning rates (e.g., $10^{-3}$ for \sevenmil vs.\ $1.6 \times 10^{-4}$ for \twobil). This confounds a pure apples-to-apples comparison of convergence dynamics, as differences in learning rate can independently affect how quickly and smoothly representations stabilise. I leave a controlled investigation of the interaction between model scale, learning rate, and convergence behaviour to future work.

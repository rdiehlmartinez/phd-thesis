...
The dataset (The Pile) is not easily streamable in its deduplicated format, and lacks a flexible preprocessing pipeline.

Training requires Megatron-DeepSpeed, which introduces heavy engineering overhead, tightly coupled dependencies, and strict hardware requirements.

The models are distributed via Docker environments with limited transparency, making it hard to debug, reproduce, or extend experiments.

Checkpointing and evaluation tools are fragmented or poorly documented, complicating integration with analysis workflows.

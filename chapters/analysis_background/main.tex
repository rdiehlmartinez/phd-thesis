...
One simple, approximate metric is the \thesishl{condition number} ($\kappa$) of a matrix. The condition number of a matrix $A$ measures how sensitive its output is to small changes in the input, and is defined as the ratio of its largest to smallest singular value ($\sigma_{max}$ and $\sigma_{min}$):
\[

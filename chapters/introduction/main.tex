...
The rapid scaling of language models has brought about remarkable capabilities, but it has also surfaced a host of challenges and risks \citep{bommasani2021foundation}. Small language models, \textit{which I define as models with fewer than 1 billion (1B) parameters}, offer a promising counterbalance. While the distinction between small and large models is relative, the 1B parameter threshold provides a practical cutoff: a model with 1B parameters requires approximately 4GB of memory (assuming 32-bit precision) and can therefore be managed on a single GPU or deployed on-device with the hardware available as of 2025. Models of this scale offer several key benefits over their large counterparts:

% The rapid scaling of language models has brought remarkable capabilities, but also surfaced significant challenges \citep{bommasani2021foundation}. Small language models—\textit{which I define as models with fewer than 1 billion (1B) parameters}—offer a promising counterbalance. A model with 1B parameters requires approximately 4GB of memory (assuming 32-bit precision) and can be managed on a single GPU or deployed on-device with hardware available as of 2025. Models at this scale offer several advantages over their larger counterparts.

\paragraph{Reduced societal and environmental risks.} Large models raise concerns about disinformation, bias amplification, and the concentration of power in a few organisations \citep{bommasani2021foundation}. They also carry substantial environmental costs; studies have quantified orders-of-magnitude differences in carbon emissions between large and small models \citep{strubell2019energy, schwartz2020greenai}. %Recent work has shown that inducing sparsity through pruning yields significant efficiency gains \citep{zhou2021hulk}.

\paragraph{Privacy preservation.} Research has shown that larger models are more prone to memorising and leaking sensitive training data \citep{carlini2021extracting, carlini2022quantifying}, with recent work establishing scaling laws for memorisation \citep{biderman2023emergent}. Smaller models, with less capacity to memorise, offer a more privacy-preserving alternative.

\paragraph{On-device deployment.} Sub-billion parameter models are essential for energy-efficient, on-device applications \citep{liu2024mobilellm}. Research teams at Apple and Google have led efforts to optimise memory bandwidth for on-device inference \citep{alizadeh2024llm} and develop custom hardware accelerators \citep{deepmind2023gemini}.

\paragraph{Interpretability.} The internal mechanisms of neural architectures are more tractable in smaller models, making them better testbeds for developing scientific understanding \citep{elhage2021mathematical, elhage2022toy}. Progress in understanding larger models often begins with insights gained from studying smaller ones.

\paragraph{Democratisation.} The cost of training large models puts them out of reach for most organisations \citep{cottier2024rising, sharir2020cost}. Minimal implementations like nanoGPT \citep{karpathy2023nanogpt} and \pico (Chapter 7) lower barriers to entry, enabling broader participation in language modelling research.
...
% By grounding small model training in insights from human cognition, this thesis seeks to move beyond ad hoc heuristics and toward a more principled, theory-driven foundation. The goal is to understand how cognitive strategies, such as curriculum learning, inductive biases, and compositional generalisation, translate into practical improvements in model design and training.
% In doing so, this work aims to bridge the gap between machine learning and cognitive science, offering not only empirical results but also conceptual tools for building small language models that are both efficient and robust.
...
\begin{itemize}
    ...
    \item \textbf{Chapter 2:} \emph{Language Modelling: Foundations, Scale, and Efficiency} offers both an overview of the development and current state of language modelling, and a review of the major strategies for training language models more efficiently. The chapter introduces foundational concepts in language modelling that are used throughout the thesis. I also draw connections between machine learning and cognitive science, considering how insights from human language acquisition can inform more principled model development, especially in resource-constrained environments.

    % I then review existing strategies for training language models more efficiently; from architectural innovations, optimisation techniques, and data calibration methods.

    \item \textbf{Chapter 3:} \emph{Curriculum Learning for Infant-inspired Model Building: A Framework for Human-like Language Acquisition} investigates curriculum learning strategies inspired by human language acquisition, focusing on their application to small language models. The work is motivated by the striking data efficiency of human language learning compared to current language models.
...
\begin{itemize}
    \item \textbf{Chapter 5:} \emph{Analysing Language Models: Foundations, Methods, and Tools} introduces the analytical foundations for studying the learning dynamics of language models. I present a range of complementary approaches for analysing these dynamics, including representational convergence and capacity usage via sparsity, norm evolution, and rank-based analysis. These methods can reveal how internal structures in language models develop, and how the learning trajectories of smaller models diverge from their larger counterparts. The chapter also situates this analysis within the broader mechanistic interpretability literature. This background chapter lays the groundwork for the next two chapters, which apply these tools to understand and intervene in the learning dynamics of small language models.

    % memorisation and data influence methods, r
...
\textbf{2. A quantitative framework for analysing learning dynamics in small models}
...
% The content of this thesis is comprised of the following published conference papers.
...
The following papers and software packages form the core contents of this thesis.

\begin{itemize}
    \item Richard Diehl Martinez, Zebulon Goriely, Hope McGovern, Christopher Davis, Andrew Caines, Paula Buttery, Lisa Beinborn (2023). {\color{thesisblue}\href{https://aclanthology.org/2023.conll-babylm.10}{CLIMB – Curriculum Learning for Infant-inspired Model Building}}. In \emph{Proceedings of the BabyLM Challenge at the 27th Conference on Computational Natural Language Learning}, pages 112--127, Singapore. Association for Computational Linguistics.

    \item Richard Diehl Martinez, Zebulon Goriely, Andrew Caines, Paula Buttery, Lisa Beinborn (2024). {\color{thesisblue}\href{https://aclanthology.org/2024.emnlp-main.344/}{Mitigating Frequency Bias and Anisotropy in Language Model Pre-Training with Syntactic Smoothing}}. In \emph{Proceedings of the 2024 Conference on Empirical Methods in Natural Language Processing}, pages 5999--6011, Miami, Florida, USA. Association for Computational Linguistics.

    \item Richard Diehl Martinez, Pietro Lesci, Paula Buttery (2024). {\color{thesisblue}\href{https://aclanthology.org/2024.findings-emnlp.187/}{Tending Towards Stability: Convergence Challenges in Small Language Models}}. In \emph{Findings of the Association for Computational Linguistics: EMNLP 2024}, pages 3275--3286, Miami, Florida, USA. Association for Computational Linguistics.


    \item Richard Diehl Martinez, David Demitri Africa, Yuval Weiss, Suchir Salhan, Ryan Daniels, Paula Buttery (2025). {\color{thesisblue}\href{https://aclanthology.org/2025.emnlp-demos.22/}{Pico: A Modular Framework for Hypothesis-Driven Small Language Model Research}}. In \emph{Proceedings of the 2025 Conference on Empirical Methods in Natural Language Processing: System Demonstrations}, pages 295--306, Suzhou, China. Association for Computational Linguistics.
...
\begin{itemize}
    ...
    \item Yuval Weiss, David Demitri Africa,  Paula Buttery, Richard Diehl Martinez (2025). {\color{thesisblue}\href{https://aclanthology.org/2025.blackboxnlp-1.9/}{Investigating ReLoRA: Effects on the Learning Dynamics of Small Language Models}}. In \emph{Proceedings of the 8th BlackboxNLP Workshop: Analyzing and Interpreting Neural Networks for NLP}, pages 163--175, Suzhou, China. Association for Computational Linguistics.
    \item David Demitri Africa, Yuval Weiss, Paula Buttery, Richard Diehl Martinez (2025). {\color{thesisblue}\href{https://aclanthology.org/2025.mrl-main.8/}{Meta-Pretraining for Zero-Shot Cross-Lingual Named Entity Recognition in Low-Resource Philippine Languages}}. In \emph{Proceedings of the 5th Workshop on Multilingual Representation Learning (MRL 2025)}, pages 106--127, Suzhou, China. Association for Computational Linguistics.
\end{itemize}
...
\begin{itemize}
    ...
    \item  Richard Diehl Martinez$^{\dagger}$, Suchir Salhan$^{\dagger}$, Zebulon Goriely, Paula Buttery (2025). {\color{thesisblue}\href{https://aclanthology.org/2025.babylm-main.10}{What is the Best Sequence Length for BabyLM?}}. In \emph{Proceedings of the First BabyLM Workshop}, pages 130--146, Suzhou, China. Association for Computational Linguistics. $^{\dagger}$Equal contribution.
\end{itemize}
\end{tcolorbox}